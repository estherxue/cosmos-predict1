...
### 量化的"锐化"效应:误差是信号相关的,不是白噪声 (2026-09-05)

用户观察:量化后的重建看起来更锐。在 visuals 四联图上量化验证(拉普拉斯方差 =
锐度;HF-corr = 高频成分与原图的对齐度;PNG 面板同一渲染管线,相对比较有效):

| clip | 原图锐度 | bf16 | INT8 mixed | full VAE |
|---|---|---|---|---|
| shooting | 13.0 | 5.7 | 6.8 (+19%) | 16.0(超原图)|
| judo | 20.9 | 13.8 | 15.5 (+12%) | 55.8(2.7× 原图)|
| drift-chicane | 41.8 | 15.8 | 17.1 (+8%) | 27.0 |
| bmx-trees | 8.7 | 3.2 | 4.7 (+47%) | 8.0 |

HF-corr 全表(重建高频与原图高频的相关度,原图 = 1.000):

| clip | bf16 | INT8 mixed | INT8 full VAE |
|---|---|---|---|
| shooting | 0.342 | 0.565 ↑ | 0.308 ↓ |
| judo | 0.549 | 0.698 ↑ | 0.363 ↓ |
| drift-chicane | 0.577 | 0.574 ≈ | 0.313 ↓ |
| bmx-trees | 0.307 | 0.407 ↑ | 0.249 ↓ |

机制三层:(1) bf16 重建本身是低通的,只保住原图 40–50% 的高频能量,这是"锐化感"
的参照系;(2) INT8 mixed 加回的高频与原图边缘**对齐度上升**(三升一平;唯一持平的
drift-chicane 是最静的 clip,bf16 基线对齐度本就最高,补边空间最小)—— 若是白噪声只会稀释下降,上升说明激活量化误差是**信号相关**的:
边缘处激活大、舍入/削顶误差大且形状随边缘,净效果类似轻度 unsharp mask;(3) 但
不是免费的细节恢复 —— 振幅/相位不对,PSNR 照掉 0.58 dB,SSIM 掉的 0.04 主要就是
这些高频扰动;人眼偏好轻锐化,所以主观观感可能反而更好。full VAE 是病态放大:
高频能量冲到原图 2.7 倍、对齐度崩掉(0.55→0.36)= 过冲/振铃/对比度失真。

含义:评估量化损伤不能只看"糊没糊"——量化不糊图,它加边缘相关的高频扰动;
主观评价(偏好轻锐化)与 PSNR/SSIM 在这里方向相反,论文口径按保真度指标报告。
